  ...
  <v-table fixed-header height="auto">
    ...
    <tbody>
      <tr v-for="memo in memos" :key="memo.id">
        ...
        <td>{{ memo.content }}</td>
        <td>
          <v-btn class="bg-success white" @click="updateTaskMemo(memo)">
  ...
  export default {
    ...
    methods: {
      ...
      confirmNewTaskMemo(memo) {
        ...
        axios
          .post(`/api/v1/tasks/${this.task.id}/memos`, memo)
          .then(() => {
            this.fetchTaskMemos();
      ...
      confirmUpdateTaskMemo(memo) {
        axios
          .put(`/api/v1/tasks/${this.task.id}/memos/${memo.id}`, memo)
          .then(() => {
            this.fetchTaskMemos();
            this.taskMemoListChildDlg = false;
          })
      ...
      confirmDelete(memo_id) {
        this.isDisabled = true;
        axios
          .delete(`/api/v1/tasks/${this.task.id}/memos/${memo_id}`)
          .then(() => {
            this.fetchTaskMemos();
          })
          .catch(error => {
